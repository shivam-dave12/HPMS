...
def _md_safe(text: str) -> str:
    """Sanitise dynamic text for use inside Markdown v1 code-spans.

    Strips characters that would break Telegram's Markdown v1 parser
    when placed inside backtick-delimited code spans.  This is the ONLY
    escaping needed inside code spans — Markdown formatting characters
    are literal within code.

    For text OUTSIDE code spans, use _mk() instead.
    """
    return (
        str(text)
        .replace("`", "")
...
class TelegramBot:
    ...
    async def _async_start(self):
        ...
        _menu_commands = [
            ...
            BotCommand("overallpnl",    "All-time P&L since bot started"),
            ...
            BotCommand("orderbook",     "Top-5 orderbook snapshot"),
            BotCommand("diag",          "Engine diagnostics"),
            ...
            BotCommand("close",         "Close current position at market"),
            ...
            BotCommand("cooldown",      "Set inter-trade cooldown"),
            BotCommand("maxloss",       "Set daily loss limit"),
            BotCommand("maxsize",       "Set max position contracts"),
            BotCommand("help",          "Command list"),
        ]
        for attempt in range(3):
            try:
                await asyncio.wait_for(
                    self._app.bot.set_my_commands(_menu_commands),
                    timeout=15.0,
                )
                logger.info("Bot command menu registered (%d commands)", len(_menu_commands))
                break
            except asyncio.TimeoutError:
                logger.warning(
                    "set_my_commands attempt %d/3 timed out", attempt + 1,
                )
            except Exception as e:
                logger.warning(
                    "set_my_commands attempt %d/3 failed: %s", attempt + 1, e,
                )
            if attempt < 2:
                await asyncio.sleep(2)
        else:
            logger.error(
                "set_my_commands FAILED after 3 attempts — bot menu will not "
                "show commands. Bot remains functional via typed commands."
            )
    ...
    def send_message(self, text: str):
        ...
        async def _send_with_fallback():
            try:
                ...
                try:
                    # Strip ALL Markdown formatting for a clean plain-text
                    # fallback.  Replace box-drawing separators with plain
                    # dashes so the message stays readable.
                    plain = text
                    for ch in ("`", "*", "_"):
    ...
    async def _safe_reply(self, update: Update, text: str):
        """Reply with Markdown, falling back to clean plain text on parse error.

        Every command handler that includes dynamic content (prices, reasons,
        engine state) MUST use this instead of raw ``reply_text`` with
        ``parse_mode="Markdown"`` to guarantee the user always sees a message
        rather than an unhandled ``BadRequest`` crash.
        """
        try:
            await update.message.reply_text(
    ...
    async def _cmd_ping(self, update: Update, ctx: ContextTypes.DEFAULT_TYPE):
        ...
        try:
            ...
            await msg.edit_text(f"🏓 Pong — {rtt:.0f}ms RTT — {_ist_now()}")
    ...
    async def _cmd_risk(self, update: Update, ctx: ContextTypes.DEFAULT_TYPE):
        ...
        try:
            ...
            await self._safe_reply(update,
                ...
                "━━━━━━━━━━━━━━━━━━━━━━━━━━\n\n"
                f"{gate_icon} *Can trade: `{_md_safe(reason)}`*\n\n"
                f"Halted:     `{_md_safe(halt_info)}`\n"
                f"Cooldown:   `{cd:.0f}s` remaining  (effective `{eff_cd}s`)\n\n"
    ...
    async def _cmd_signal(self, update: Update, ctx: ContextTypes.DEFAULT_TYPE):
        ...
        await self._safe_reply(update,
            ...
            f"p final:    `{s.predicted_p_final:.6f}`\n"
    ...
    async def _cmd_pnl(self, update: Update, ctx: ContextTypes.DEFAULT_TYPE):
        ...
        halted_txt = (
            "Yes — " + _md_safe(r.get("halt_reason", ""))
            if r.get("is_halted") else "No"
        )
    ...
    async def _cmd_position(self, update: Update, ctx: ContextTypes.DEFAULT_TYPE):
        ...
        await self._safe_reply(update,
            ...
            f"{pnl_block}\n\n"
            f"SL order: `{_md_safe(str(p.get('sl_order') or 'N/A'))}`\n"
            f"TP order: `{_md_safe(str(p.get('tp_order') or 'N/A'))}`",
        )
    ...
    async def _cmd_start_trading(self, update: Update, ctx: ContextTypes.DEFAULT_TYPE):
        ...
        if self._risk:
            ...
            if rs.get("is_halted"):
                await self._safe_reply(update,
                    f"⚠️ Risk manager is HALTED (`{_md_safe(rs['halt_reason'])}`)\n"
    ...
    async def _cmd_halt(self, update: Update, ctx: ContextTypes.DEFAULT_TYPE):
        ...
        actions_txt = "\n".join(
            f"• {_md_safe(a)}" for a in actions
        ) if actions else "• No actions needed"
        await self._safe_reply(update,
            f"⛔ *EMERGENCY HALT*  ⏱ `{_ist_now()}`\n\n"
            f"Actions taken:\n{actions_txt}\n\n"
            f"_Use /resume then /start\\_trading to re-enable._",
        )
    ...
    async def _cmd_resume(self, update: Update, ctx: ContextTypes.DEFAULT_TYPE):
        ...
        await self._safe_reply(update,
            ...
            f"Was halted: `{_md_safe(str(was or 'none'))}`\n"
    ...
    async def _cmd_resetrisk(self, update: Update, ctx: ContextTypes.DEFAULT_TYPE):
        ...
        await self._safe_reply(update,
            ...
            f"Was: `{_md_safe(str(was or 'none'))}`  →  Consecutive losses: `0`\n"
    ...
    async def _cmd_flatten(self, update: Update, ctx: ContextTypes.DEFAULT_TYPE):
        ...
        actions = result.get("actions", [])
        actions_txt = (
            "\n".join(f"• {_md_safe(a)}" for a in actions)
    ...
    async def _cmd_close(self, update: Update, ctx: ContextTypes.DEFAULT_TYPE):
        ...
        if result.get("success"):
            ...
            await self._safe_reply(update,
                f"❌ Close failed: `{_md_safe(str(result.get('error')))}`",
            )
        ...
        if self._api:
            ...
            try:
                ...
                if result.get("success"):
                    ...
                    lines.append(f"❌ Exchange rejected: `{_md_safe(str(result.get('error', 'unknown')))}`")
                    await self._safe_reply(update, "\n".join(lines))
                    return False
            except Exception as e:
                lines.append(f"❌ Exchange call failed: `{_md_safe(str(e))}`")
                await self._safe_reply(update, "\n".join(lines))
                return False
        else:
