...
class RiskManager:
    ...
    def _reset_if_new_day(self):
        ...
        if today != self._day_start:
            ...
            if self._is_halted and "DAILY" in self._halt_reason:
                ...
                self._halt_reason = ""
            logger.info(f"RiskManager daily reset: {today}")
    ...
    def can_trade(self) -> Tuple[bool, str]:
        ...
        with self._lock:
            ...
            if self._open_trade and not self._open_trade.closed:
                return False, "POSITION_OPEN: already in a trade"

            # Cooldown
            elapsed = time.time() - self._last_trade_time
            if elapsed < self._cooldown:
                return False, f"COOLDOWN: {self._cooldown - elapsed:.1f}s remaining"
            ...
            if self._daily_pnl <= -self._max_daily_loss:
                ...
                return False, f"DAILY_LOSS: ${self._daily_pnl:.2f} <= -${self._max_daily_loss:.2f}"

            # Consecutive losses
            if self._consecutive_losses >= self._max_consec_losses:
                self._halt("CONSECUTIVE_LOSSES")
                return False, f"CONSEC_LOSSES: {self._consecutive_losses}/{self._max_consec_losses}"
            ...
            if self._session_high_pnl > 0:
                ...
                if dd > self._max_dd_pct:
                    ...
                    return False, f"DRAWDOWN: {dd:.1f}% > {self._max_dd_pct}%"
    ...
    def compute_size(self, price: float, equity_usd: float) -> int:
        ...
        Uses Kelly-lite: risk equity_pct% of equity, capped by max notional and
        max contracts.
        """
        with self._lock:
    ...
    def check_exit_conditions(self, dH_dt: float, dH_dt_spike: float, bars_held: int, max_hold: int) -> Optional[str]:
